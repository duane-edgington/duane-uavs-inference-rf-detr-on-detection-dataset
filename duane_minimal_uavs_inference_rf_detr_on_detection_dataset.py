# -*- coding: utf-8 -*-
"""duane-minimal-uavs-inference-rf-detr-on-detection-dataset.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/14XLtG7f6NUTnSfLM6JG9jb1-7EVrlWq4

[![Roboflow Notebooks](https://media.roboflow.com/notebooks/template/bannertest2-2.png?ik-sdk-version=javascript-1.4.3&updatedAt=1672932710194)](https://github.com/roboflow/notebooks)

# Inference, minimal code, using  RF-DETR Object Detection model on a Custom Dataset
...
# just import a model so we can cleanup the gpu memory
...
# Install SAHI with the Roboflow integration
# (rfdetr is already installed by your earlier pip cell, keep that unchanged)
# -----------------------------------------------------------------------------

!pip install -q "rfdetr[onnxexport]==1.5.1" "sahi[roboflow]"

!pip show rfdetr | grep Version
!pip show sahi | grep Version
...
methods = [m for m in dir(export_model) if not m.startswith('_')]
print(methods)
...
print(f"Copied to {ONNX_MODEL_PATH}")

#!pip install -q --upgrade rfdetr

# ─── NEW CELL 2 ──────────────────────────────────────────────────────────────
# Export the trained .pt checkpoint to ONNX format,
# and read the class list from _annotations.coco.json
# -----------------------------------------------------------------------------
...
if os.path.exists(ONNX_MODEL_PATH):
    ...
    print(f"\nExporting to {ONNX_MODEL_PATH} ...")
    export_model = RFDETRLarge(
        pretrain_weights=PT_WEIGHTS,
        resolution=896,
        num_classes=len(CLASS_NAMES),
    )
    export_model.export(output_path=ONNX_MODEL_PATH)   # <-- correct method name
    del export_model
    gc.collect()
    torch.cuda.empty_cache()
...
class RFDetrOnnxDetectionModel(DetectionModel):
    ...
    def convert_original_predictions(self, shift_amount=None, full_shape=None):
        ...
        # scores = sigmoid(logits), squeeze the last dim
...
img = Image.open(os.path.join(TEST_IMAGES_DIR, test_images[0])).convert("RGB")
...
result = get_sliced_prediction(
    ...
    verbose=2,   # <-- shows per-tile progress so we can see where it hangs
)
t1 = time.time()
print(f"\nDone in {t1-t0:.1f}s")
...
# Run inference on just one tile to inspect raw outputs
test_images = sorted([f for f in os.listdir(TEST_IMAGES_DIR) if f.lower().endswith(('.jpg','.jpeg','.png'))])
img = np.array(Image.open(os.path.join(TEST_IMAGES_DIR, test_images[0])).convert("RGB"))

# Crop one tile manually
tile = img[:896, :896]

# Run through the ONNX session directly
tile_pil = Image.fromarray(tile).resize((896, 896), Image.BILINEAR)
tile_np = np.array(tile_pil, dtype=np.float32) / 255.0
mean = np.array([0.485, 0.456, 0.406], dtype=np.float32)
std  = np.array([0.229, 0.224, 0.225], dtype=np.float32)
tile_np = (tile_np - mean) / std
tile_np = tile_np.transpose(2, 0, 1)[None]

outputs = detection_model.session.run(
    detection_model.output_names,
    {detection_model.input_name: tile_np}
)

for name, out in zip(detection_model.output_names, outputs):
    print(f"{name}: shape={out.shape}, dtype={out.dtype}")
    print(f"  min={out.min():.4f}, max={out.max():.4f}, mean={out.mean():.4f}")
    print(f"  first row: {out[0][0]}")
...
for img_filename in tqdm(test_images, desc="Processing images"):
    ...
    ## updated to use SAHI over onnx model
    ...
    # clamp negative class_ids that can come from the ONNX labels output
...
# show the 5 images with most detections
...
if os.path.exists(sample_img_path):
        ...
        display(Image.open(sample_img_path))
        print(f"\n📸 Preview: {test_images[17]} (showing annotated image)")
